...
def end_game():
    ...
    if total_shots == 0:
        ...
        intro += str(min(100.0 * float(score) / float(total_shots), 100.0)) + '%'
    text(intro, (width/3) + 100, (height/2) + 100)
    text("shots per second:    " + str(float(total_shots)/30), (width/3) + 100, (height/2) + 275)
    text("aliens shot per second:    " + str(float(score)/30), (width/3) + 100, (height/2) + 320)
    text("press ENTER to restart", width/3 + 100, height/2 + 365)
    pop()
